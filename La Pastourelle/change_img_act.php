...
if (! isset($_SESSION['pseudo']) or ! isset($_SESSION['pass']) or
    ...
    if (isset($_FILES['fichier_choisi']) and isset($_POST["repertoire"]) and
             isset($_POST["nom"]) and isset($_POST["page_lang"])) {
        transfert();
        $_POST["photo"] = "";
    }
    ?>
...
<form name="formulaire" method="POST" enctype="multipart/form-data"
	action="index.php?page=change_img_act">
    ...
    for ($i = 0; $i < count($allLang); $i ++) {
        if ($allLang[$i]['lang'] == $_SESSION['lang']) {
            echo '<input type="radio" name="page_lang" value="' .
                     $allLang[$i]['lang'] . '" checked="checked"/>';
        } else {
            echo '<input type="radio" name="page_lang" value="' .
                     $allLang[$i]['lang'] . '" />';
        }
        if (! file_exists('image/lang/' . $allLang[$i]['lang'] . '.png')) {
            echo "<label for=page_lang><img src='image/lang/inc.png'  /></label>";
        } else {
            echo "<label for=page_lang><img src='image/lang/" .
                     $allLang[$i]['lang'] . ".png'  /></label>";
        }
    }
    ?>
				<br />
	<br /> <label for='photo'>Choisissez une photo : </label> <select
		name="photo">
		<option>danse</option>
		<option>theatre</option>
	</select> <INPUT type="submit" value="Sélectionner"><br><?php
    if (isset($_POST["photo"]) and ! empty($_POST["photo"]) and
             isset($_POST["page_lang"])) {
...
<br>
<br>
<A class="btn btn-link" HREF="index.php?page=adminActualite">Retour à la
	page précédente</A>
<?php
}
?>