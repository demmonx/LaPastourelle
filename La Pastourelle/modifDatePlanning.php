...
<?php
	...
	if (count ( $info_date ) <= 0) {
		...
		echo "<BR><BR>";
		echo "<FORM METHOD='POST' id='modif' ACTION='modifDatePlanningBD.php'>
			<TABLE WIDTH=840px>
				<TR><TD>Jour</TD><TD><INPUT required type=text id='jour' name='jour' size=15px value='" . $info_date ['jour'] . "'></TD></tr>
			<tr><TD>Date (jj/mm/aaaa)</TD><TD><INPUT required type=text id='date' name='date' size=15px value='" . $info_date ['date'] . "'></TD></tr>
			<tr><TD>Lieu</TD><TD><INPUT required type=text name='lieu' id='lieu' size=15px value='" . $info_date ['lieu'] . "'></tr>
			<tr><TD>Musiciens</TD><TD><textarea required name='musiciens' id='joueur'>" . $info_date ['joueur'] . "</textarea></TD></TR></TABLE>
