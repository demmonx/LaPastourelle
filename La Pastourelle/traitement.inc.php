...
/**
 * Retourne tous les continents dans lesquels un voyages à été effectué par la pastourelle
 */
function getVoyage() {
	$bdd = new Connection ();
	$return = array ();
 ...
 * @param array $row Une ligne de BD
 ...
 * @param array $serveur La variable superglobales serveur
 * @param string $valeur La valeur du code de sécurité
...
function getMembers() {
	...
	$stmt = $bdd->prepare ( "SELECT * FROM tuser WHERE etat_validation = 1 AND niveau=0 ORDER BY nom" );
	$result = $stmt->execute ();
	$i = 0;
	while ( $row = $stmt->fetch () ) {
...
function getLanguages() {
	...
	$bdd = new Connection ();
	$return = array ();
	$stmt = $bdd->prepare ( "SELECT * FROM lang JOIN languages_world ON lang_code = id" );
	$stmt->execute ();
	while ( $row = $stmt->fetch () ) {
...
function reverseLanguage($code) {
	$bdd = new Connection ();
	$stmt = $bdd->prepare ( "SELECT lang_id FROM lang JOIN languages_world ON lang_code=id WHERE locale=?" );
...
function updateProduct($nom, $desc, $produit, $langue) {
	...
	$stmt1 = $bdd->prepare ( "SELECT * FROM produits_contenu
			JOIN produits
			ON pd_num = bt_prod
	...
	if ($stmt1->rowCount () == 0 && ! empty ( trim ( $nom ) )) {
		$stmt2 = $bdd->prepare ( "INSERT INTO produits_contenu
		...
		$stmt2->execute ();
		return true;
	} else if ($stmt1->rowCount () > 1) { // Plusieurs items avec même clé =
	                              // table corrompue
		throw new InvalidArgumentException ( "Au moins une donnée est corrompue" );
	} // sinon on update
	
	$stmt2 = $bdd->prepare ( "UPDATE produits_contenu
			SET bt_content = :desc,
            bt_nom_public = :nom
			WHERE bt_num = :id" );
	$stmt2->bindValue ( ":desc", ! empty ( $desc ) ? $desc : null );
...
function addProduct($nom, $prix) {
	...
	$stmt = $bdd->prepare ( "INSERT INTO produits (pd_nom_prive, pd_nom_admin, pd_prix) VALUES(?,?,?)" );
	$stmt->bindValue ( 1, $type );
	$stmt->bindValue ( 2, $nom );
...
function addLink($nom, $url) {
	...
	$stmt = $bdd->prepare ( "INSERT INTO lien_ext (lien_url, lien_nom) VALUES(?,?)" );
	$stmt->bindValue ( 1, $url );
	$stmt->bindValue ( 2, $nom );
	$stmt->execute ();
}
...
function deleteImageProduct($id) {
	...
	if ($stmt1->rowCount () == 1) {
		...
		if (isset ( $row ['pd_img'] )) {
			$stmt2 = $bdd->prepare ( "UPDATE produits SET pd_img = null WHERE pd_num = ?" );
			$stmt2->bindValue ( 1, $id );
			$stmt2->execute ();
			@unlink ( $row ['pd_img'] );
		}
...
function deleteImageLink($id) {
	...
	if ($stmt1->rowCount () == 1) {
		...
		if (isset ( $row ['lien_img'] )) {
			$stmt2 = $bdd->prepare ( "UPDATE lien_ext SET lien_img = null WHERE lien_num = ?" );
			$stmt2->bindValue ( 1, $id );
			$stmt2->execute ();
			@unlink ( $row ['lien_img'] );
		}
...
function insertImageLink($image, $id) {
	$bdd = new Connection ();
	deleteImageLink ( $id );
	$stmt2 = $bdd->prepare ( "UPDATE lien_ext SET lien_img = ? WHERE lien_num = ?" );
	$stmt2->bindValue ( 1, $image );
	$stmt2->bindValue ( 2, $id );
	$stmt2->execute ();
	return true;
...
function decodeREGEX($str) {
	$str = preg_replace ( '`\{image}(.+)\{/image\}`iUs', '<img src="$1" alt="Image" /> ', $str );
	...
	$str = preg_replace ( '`\{video}(.+)\{/video\}`iUs', "<video controls src='$1'></video>", $str );
...
function setContent($page, $lang, $content) {
	...
	// On regarde si la valeur existe déjà
	$stmt1 = $bdd->prepare ( "SELECT * FROM texte
			JOIN page
			ON txt_page = page_id
			WHERE page_id = :page
			AND lang = :lang" );
	...
	// sinon inutile
	if ($stmt1->rowCount () == 0 && ! empty ( trim ( $content ) )) {
		$stmt2 = $bdd->prepare ( "INSERT INTO texte
	...
	$stmt2 = $bdd->prepare ( "UPDATE texte
			SET texte = :valeur
			WHERE txt_num = :id" );
	$stmt2->bindValue ( ":valeur", $content );
...
function setCompteRendu($id, $content) {
	$bdd = new Connection ();
	$stmt = $bdd->prepare ( "UPDATE compte_rendu
			SET cr_text = :valeur
			WHERE cr_num = :id" );
...
function addCompteRendu($content, $date) {
	...
	$bdd = new Connection ();
	$stmt = $bdd->prepare ( "INSERT INTO compte_rendu (cr_text, cr_date) VALUES (:content, :date)" );
...
function deleteFile($id) {
	$bdd = new Connection ();
	$stmt1 = $bdd->prepare ( "SELECT file_adr FROM uploaded_file WHERE file_num = ?" );
	$stmt1->bindValue ( 1, $id );
	$stmt1->execute ();
	if ($stmt1->rowCount () == 1) {
...
function deleteRevue($id) {
	$bdd = new Connection ();
	$stmt1 = $bdd->prepare ( "SELECT presse_img FROM revue_presse WHERE presse_num = ?" );
	$stmt1->bindValue ( 1, $id );
	$stmt1->execute ();
	if ($stmt1->rowCount () == 1) {
...
function addDatePlanning($jour, $date, $lieu, $musiciens) {
	$bdd = new Connection ();
	$stmt = $bdd->prepare ( "INSERT INTO planning (pl_jour, pl_date, pl_lieu, pl_musiciens) 
...
function addVoyage($pays, $titre, $texte) {
	$bdd = new Connection ();
	$addVoy = $bdd->prepare ( "INSERT INTO logocarte (pays, titre, texte)
				VALUES(?, ?, ?)" );
